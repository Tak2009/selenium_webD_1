...
describe("First name field check", () => {
  ...
  it("blank case", async () => {
    ...
    assert(await driver.wait(until.elementLocated(By.css(".atm-error-message-container"),10000)).getText() == "お客様のお名前をご教示下さい。")
  });
  ...
  it("number case", async () => {
    ...
    assert(await driver.wait(until.elementLocated(By.css(".atm-error-message-container"),10000)).getText() == "お名前が正しくないフォーマットで記入されています。半角の英字でご記入下さい。")
  });
